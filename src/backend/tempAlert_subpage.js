...
class TempAlertSubpage extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      templateList: [],
      selTemplate: { alert_rule_id: "sys_temperature", code: "", monitor_rule_id: "", name: "", rule_template_id: "" },
      selTempMonitor: -30,
      /*{
            interval: 10,
            item: "temperature",
  ...
  handleOpenModal = (e, data) => {
    ...
    let {template, selTempMonitor, isModify, alert_rule_id} = data;
    template.alert_rule_id = alert_rule_id;
  ...
  handleItemChanged = (e,type) => {
    switch (type){
      case "template":
        ...
        break;
      case "monitor":
        //console.log("this.dropdownTempMonitor",this.dropdownTempMonitor);
        var condition = this.dropdownTempMonitor.filter(con => {
          return (con.upper_limit == e.value);
        });
        let monitor_rule_id = "";
  ...
  handelAddNewItem = async (e) =>{
    ...
    if( this.state.selTemplate.name.trim() == "" ) {
      ...
      if(this.state.selTemplate.monitor_rule_id == "") {
        var condition = this.dropdownTempMonitor.filter(con => {
          return (con.upper_limit == this.state.selTempMonitor);
        });
        if(condition.length > 0) {
          monitor_rule_id = condition[0].monitor_rule_id;
        } else {
      ...
      if(this.state.isModify)
      {
        ...
        //reqData.rule_template.alert_rule_id = this.state.alert_rule_id
  ...
  doGetMonitorList = () => {
    ...
    .then(function(response) {
      if(response.data.status == 1) {
        ...
        sortByKey(monitor, "upper_limit", false, false, false);
        _.map(monitor, list=>{
          if(list.item == "temperature") {
  ...
  doAddMonitorItem = async (upperlimit) =>{
    let monitor_rule = {
      code: upperlimit,
      name: upperlimit+"℃",
      item: "temperature",
  ...
  doGetTemplateList = () => {
    ...
    .then(async function(response) {
      if(response.data.status == 1) {
        ...
        if(list.length == 0) {
          console.log("Add default temp alert!");
          console.log("this.state.alert_rule_id:",this.state.alert_rule_id);
          let monitor_rule_id = await this.doAddMonitorItem(30);
          this.doGetMonitorList();
          let reqData = {
            rule_template:{
              alert_rule_id: this.state.alert_rule_id,
  ...
  render() {
    ...
    let viewWidth = window.innerWidth - 430;
    return (
      <div className="Subpage">
        { this.renderConfirmDialog() }
        <div className="WhiteBGSubpage">
          <Button style={{float:"right",marginRight:"30px",cursor:"pointer", marginBottom:"5px"}}
                  onClick={e=>this.handleOpenModal(e,{template:{alert_rule_id:this.state.alert_rule_id,code:"",monitor_rule_id:"",name:"",rule_template_id:""},
                    selTempMonitor:-30,isModify:false})}>{Locales.elecAlert.新增警示群組}
          </Button>
          <AddAlertGroupModal
          ...
          {this.state.alert_rule_id == "" && <ConfirmDialog
            content={"Temperature Custom does not exist  in the rule_template Alert list "}
          />}
            ...
            <table style={{width:"100%", textAlign: "center"}}>
              <tbody>
                <tr className="rowDataHeader">
                  ...
                  <td width={`${viewWidth*30/100}px`}>{Locales.elecAlert.溫度條件}</td>
                  <td width={`${viewWidth*20/100+25}px`}></td>
                </tr>
              </tbody>
              <tbody style={{ maxHeight: ((height-70) + "px")}}>
                <TempAlertData
    ...
    <Modal
      ...
      <table width="100%" className="CorrectSubpageTable">
        <tbody>
          ...
          <tr>
            <td width="100%">
              <input autoFocus type="text" className={"InputStyle "+chTempName} maxLength="30"
                     onChange={e=>props.onValueChanged(e,'template')} value={typeof props.selTemplate.name == "undefined" ? "" : props.selTemplate.name}></input>
              {/*<div style={{color:"red"}}>{props.alertContent}</div>*/}
            </td>
          </tr>
          <tr style={{height:"10px"}}>
           <td width="100%">{Locales.elecAlert.溫度條件}</td>
          </tr>
          <tr>
            <td width="100%">
              <DropdownButton Title={(typeof props.selTempMonitor == "undefined") ? "" : props.selTempMonitor}
                              Id="1" MenuItems={props.tempMenuItems} onSelectChanged={props.onValueChanged} type="monitor"/>
            </td>
          </tr>
        </tbody>
      </table>
...
function TempAlertData(props) {
  const datalist = props.templateList;
  //console.log("props.tempMenuItems:",props.tempMenuItems);
  let viewWidth = window.innerWidth - 430;
  return _.map(datalist, data => {
    var condition = props.tempMenuItems.filter(con => {
      return (con.monitor_rule_id == data.monitor_rule_id);
    });
    //console.log("data.monitor_rule_id:",data.monitor_rule_id);
    //console.log("condition:",condition);
    return (
      <tr key={data.rule_template_id} className="rowDataContent">
        ...
        <td width={`${viewWidth*20/100}px`} className="tableBtnRow">
          <Button onClick={e=>props.onShowModal(e,{template:data,selTempMonitor:(typeof condition[0]!="undefined") ? condition[0].upper_limit:"",isModify:true})}>
            {Locales.common.修改}
          </Button>{" "}
          <Button className="btn btn-danger" disabled={data.code=="0"} onClick={e=>props.handleDelete(e,data)}>{Locales.common.刪除}</Button>
        </td>
      </tr>
    );
  });
...
function DropdownButton(props) {
  let options=[];
  for(let i=-30 ; i<=30 ; i++) {//for(let i=0; i<props.MenuItems.length;i++){
...
export default connect(mapStateToProps, {  })(TempAlertSubpage);
